...
- **后端日志**:`webui\data\logs\backend.log`(5MB×4 份自动轮转),任务生命周期、引擎拉起/退出、
  异常堆栈都有带时间戳记录;启动窗口同步显示。网页「总览」页右上角「系统日志」可直接翻看
  后端 / 提取引擎日志尾部(3s 自动刷新)。
- **任务日志**:每个任务的原始输出在 `webui\data\logs\<任务ID>.log`(任务详情抽屉内直接查看),
  保留 30 天。
- **提取引擎日志**:`webui\data\logs\worker.log`(超 10MB 自动轮转为 `.1`)。
- **日志级别**:环境变量 `WM2_LOG_LEVEL`(默认 INFO,排障设 DEBUG;可在 `webui\local_config.bat`
  里 `set "WM2_LOG_LEVEL=DEBUG"`,重启生效)。
- **命令行长任务**:`src\`、`tools\` 下的脚本均支持 `--log-file 路径`,事件流水带时间戳写入文件,
  控制台输出不变;不给参数则行为与原来完全一致。
